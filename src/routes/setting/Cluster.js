...
class Cluster extends Component{
  ...
  componentDidMount(){
    ...
    if(base.currentUser.name === 'admin'){
      this.setState({ showPublicCluster:true })
    }else{
      this.setState({ showPublicCluster:false })
    }
  }
    ...
    return (
        ...
        <div className='cardList'>
          ...
          renderItem={(item,i)  => (!item.new?
              ...
              <Card hoverable className='card' actions={item.public?(this.state.showPublicCluster?[<a onClick={e=>this.handleAddClick(e,item)}>新增主机</a>, <a onClick={e=>this.handleDelete(e,item)}>删除集群</a>]:['']):[<a onClick={e=>this.handleAddClick(e,item)}>新增主机</a>, <a onClick={e=>this.handleDelete(e,item)}>删除集群</a>]}>
                <Card.Meta 
