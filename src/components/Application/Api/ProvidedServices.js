...
//this.props.editable false 是全局服务列表，true 是应用详情里面的服务列表
class ProvidedServices extends React.Component {
    ...
    componentDidMount() {
        if(this.props.editable){
            ...
            queryAllGroups(this.props.tenant).then(data=>{
    ...
    componentWillReceiveProps(nextProps){
        if((nextProps.tenant && nextProps.tenant !== this.props.tenant) || (nextProps.environment !== this.props.environment)){
            queryAllGroups(nextProps.tenant).then(data=>{
                this.data = data.slice();
                let treeData = TreeHelp.toChildrenStruct(data);
                this.setState({ treeData });
            });
    ...
    _pullData(groupId, page, rows,tenant) {
        ...
        queryAllServices(groupId,page,rows,value,filterMethod,name,uri,tenant).then(response=>{
            let pagination = { current: response.pageIndex,totalPage:response.totalPage, total: response.total, pageSize: response.pageSize, pageSizeOptions: ['10', '20', '30', '50'], showSizeChanger: true, showQuickJumper: true, };
            this.setState({
    ...
    _getColumn() {
        return [{
            ...
            render: (text, record) => {
                return (
                    <Link to={`/apps/${this.props.appId}/apis/${record.id}`}><Ellipsis style={{ margingLeft: 5 }} lines={1} tooltip={true}>{text}</Ellipsis></Link>
                )
            }
        }, {
            title: '方法',
            ...
            <div>
                ...
                <Row>
                    <Row type={'flex'} style={{ marginBottom: 16 }}>
                        ...
                        <Button onClick={() => this.onDebugger() }>在线文档</Button>
                        <Modal
                            title="文档预览"
                            okText='预览'
                            ...
                            {!this.props.editable?
                                <Row style={{ marginBottom:24 }} type={'flex'} align='middle'>
                                    <Col style={{ textAlign:'right',paddingRight:8 }} span={6}>服务组:</Col><Col span={18}>
                                    <TreeSelect
                                        style={{ width: '100%' }}
