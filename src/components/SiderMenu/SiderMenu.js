...
let currentPath,tempMenuTenantDisabled;
export default class SiderMenu extends React.PureComponent {
  ...
  componentWillReceiveProps(nextProps){
    ...
    if(path !== currentPath || tempMenuTenantDisabled!==constants.MENUS[4].disabled){
      this.menuInit();
    }
  }
  componentDidMount(){
    this.menuInit();
    tempMenuTenantDisabled = constants.MENUS[4].disabled;
  }
  //菜单展开选中初始化
  menuInit = ()=>{
    const path = window.location.hash.substring(1);
