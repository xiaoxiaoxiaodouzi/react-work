...
describe('test getBreadcrumb', function () {
  it('Simple url', function () {
    expect((0, _index.getBreadcrumb)(routerData, '/dashboard/analysis').name).toEqual('分析页');
  });
  it('Parameters url', function () {
    expect((0, _index.getBreadcrumb)(routerData, '/userinfo/2144').name).toEqual('用户信息');
  });
  it('The middle parameter url', function () {
    expect((0, _index.getBreadcrumb)(routerData, '/userinfo/2144/addr').name).toEqual('收货订单');
  });
  it('Loop through the parameters', function () {
    var urlNameList = (0, _pathTools.urlToList)('/userinfo/2144/addr').map(function (url) {
      return (0, _index.getBreadcrumb)(routerData, url).name;
    });
