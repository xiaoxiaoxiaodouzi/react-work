...
export function addServers(appid, upstream, contents, json, mark) {
    return C2Fetch.post(proxy + `aip/v1/import/${appid}/services`, { contents: contents, json: json, mark: mark, upstream: upstream ,tenant:base.currentEnvironment}, {}, "导入服务失败");
}
...
export function addService(appid,params) {
    let url = proxy + `aip/v1/groups/${appid}/services?ctx=`;
    return C2Fetch.post(url,params,null,"添加服务失败");
}
//获取应用服务组失败
export function queryAllGroups(){
    let url = proxy + 'aip/v2/groups';
...
export function queryAllServices(groupId,page,rows,tagId,method,name,uri){
    let params = {
        page:page,
        rows:rows,
        groupId:groupId,
