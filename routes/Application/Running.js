...
class Running extends Component {
	...
	loadData = () => {
		...
		if (data) {
			...
			this.setState({
				appType: data.type, appDatas: data, type: data.type === 'middleware' ? '中间件' : '应用', springCloud: data.springcloud ? true : false,
	...
	handleOk1 = (e) => {
		...
		this.props.form.validateFields((err, values) => {
			...
			if (this.state.switch && !this.state.isOpen) {
	...
	render() {
		...
		return (
			<div>
				...
				<Card bordered={false} style={{ margin: 24 }} title='服务器地址'>
					...
					this.state.isOpen ?
						<div>
							...
							<Card title="可访问应用的用户列表" style={{ margin: 24 }} bordered={false}>
								<User appId={this.props.match.params.id} />
							</Card>
						</div>
						: null
				}
				...
				>
					<Form >
						...
						<FormItem {...formItemLayout} label="匿名访问地址" >
							{getFieldDecorator('anonurls', { initialValue: this.state.anonurls })(
								<Input placeholder="输入地址" />
							)}
						</FormItem>
					</Form>
...
export default ErrorComponentCatch(Form.create()(Running));