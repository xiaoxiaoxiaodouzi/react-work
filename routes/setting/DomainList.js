...
class DomainListForm extends Component {
  ...
  handleEdit = (record) => {
    ...
    const protocols = Array.isArray(record.protocols)&&record.prevProps.length>0?record.protocols[0].toLowrCase():'http';
    setFieldsValue({
  ...
  render() {
    return (
      <div style={{ margin: '-24px -24px 0' }}>
        <PageHeaderBreadcrumb breadcrumbList={[{name:'平台管理'},{name:'域名管理'}]} action={<GlobalEnvironmentChange/>}/>
        <div style={{ margin: '24px 24px 0' }}>
          {this.renderOpen()}
        </div>
      </div>
