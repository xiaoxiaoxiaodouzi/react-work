...
import {Card,List,Button,Icon,Modal,Input,message,Table,Popconfirm,Row,Col} from 'antd'
import './index.less'
import {getClusters,addCluster,deleteCluster,getNodes,deleteNodes,addHosts} from '../../services/cluster'
...
class Cluster extends Component{
      ...
      },
      {
        ...
        render:(text,record)=>{
          if(this.props.public){
            ...
            return <Popconfirm title='确认删除？' onConfirm={e=>this.handleListDelete(e,record.name)}>
                        <a>移除</a>
                    </Popconfirm> 
          }
        }
    ...
    return (
        ...
        <div className='cardList'>
          ...
          renderItem={(item,i)  => (!item.new?
              ...
              <Card hoverable className='card' actions={item.public?[' ']  :[<a onClick={e=>this.handleAddClick(e,item)}>新增主机</a>, <a onClick={e=>this.handleDelete(e,item)}>删除集群</a>]}>
                <Card.Meta 
        ...
        <pre className='preStyle' >{
          `curl -sS http://s3.c2cloud.cn/k8s-builder/install-docker.sh | sh -s 172.17.80.39`
  ...
  sudo systemctl restart docker            # Docker重启l=true 
    --storage-opt=dm.use_deferred_deletion=true`}
        ...
        <pre className='preStyle'>
            {`curl -sS http://s3.c2cloud.cn/k8s-builder/install.sh | sh -s 4611a2.671eaca61a789a3b
               `} 
            {this.state.ip}  {`  172.17.80.39`}
        </pre>
        <p>
