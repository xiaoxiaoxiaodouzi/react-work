...
class SafeStrategyComponent extends Component {
                        ...
                        <Form.Item style={{ height: '100%' }}>
                            ...
                            {getFieldDecorator(array.code, {
                                ...
                                {array.items.map((item) => {
                                    return (<Option key={array.code} value={item.value}><Tooltip title={item.name}>{item.name}</Tooltip></Option>)
                                })}
                            </Select>)}
                        </Form.Item>
                    </Col>
    ...
    render() {
        // const breadcrumbTitle = BreadcrumbTitle([{name:'平台管理'},{name:'安全策略'}]);
