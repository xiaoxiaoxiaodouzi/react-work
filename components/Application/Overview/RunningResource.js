...
class RunningResource extends React.PureComponent {
  ...
  loadData = (props) => {
    ...
    if (appData) {
      ...
      deployMode = appData.deployMode;
      appId = appData.id;
    }
    if (appCode) {
      this.setState({
        deployMode: deployMode, appCode: appCode,
        APMChecked: props.APMChecked, appId: appId,
        visible: deployMode === "k8s" && base.configs.passEnabled,
        currentTabKey: props.APMChecked && base.configs.APMEnabled ? 'performance' : 'resource'
      });
    }
  }
                ...
                <TabPane tab="性能监控" key="performance" disabled={this.props.APM_URL ? false : true}>
                  {this.state.APMChecked ?
                    <div style={{ padding: '0px 24px 0px 24px' }}>
                      ...
                      <RequestTime appCode={this.state.appCode} rangePickerValue={rangePickerValue} />
                      <Divider style={{ marginBottom: 24, marginTop: 24 }} />
                      <Row gutter={24}>
                        <Col span={8}>
                          <div className="card-title">应用响应时间分布</div>
                          <ResponseTime appCode={this.state.appCode} rangePickerValue={rangePickerValue} />
                        </Col>
                        <Col span={16}>
                          <div className="card-title">TOP10响应慢服务</div>
                          <SlowestTransaction appCode={this.state.appCode}
                            from={moment(rangePickerValue[0]).format('x')}
                            to={moment(rangePickerValue[1]).format('x')}
