...
class Entrance extends Component {
	...
	handleFieldChange(e, fieldName, record) {
		...
		if (target) {
			target[fieldName] = e.target.value || e.target.checked;
			this.setState({ data: newData });
		}
	}
	...
	render() {
		const columns = [
			...
			{
				title: '关注',
				dataIndex: 'defaultEntry',
				width: '10%',
				render: (text, record) => {
					if (record.editable) {
						return (
							<Checkbox defaultChecked={text} onChange={e => this.handleFieldChange(e, 'defaultEntry', record)}>是</Checkbox>
						);
					} else {
						return text ? '是' : '否';
					}
