...
class Step2 extends React.Component {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				if (containers.length > 0) {
					...
					let clusterPorts = this.props.clusterService.spec.ports;
					let nodePortPorts = this.props.nodePortService.spec.ports;
					nodePortPorts = [];
					containers.forEach(c => {
						...
						c.config.forEach(c => {
							...
							this.props.configMap.data[c.key] = c.contents;
						})
					})
				} else {
	...
	render() {
		...
		return (
			<div style={{ display: this.props.displayStep2 ? 'block' : 'none' }}>
				<Row>
...
const StepTwo = Form.create()(Step2);
export default props => (
	<CreateAppContext.Consumer>
		{context => <StepTwo {...props} {...context} />}
	</CreateAppContext.Consumer>
)