...
class DeployConfig extends Component {
	...
	static getDerivedStateFromProps(nextProps, prevState){
		if(nextProps.item && nextProps.item !== prevState.item){
	...
	componentDidUpdate(prevProps, prevState){
		if(prevState.item){
			getEnvs(prevState.item.namespace, prevState.item.name).then(data => {
				let ary = [];
				...
				return ({ envConfData: ary ,item:prevState.item})
			})
		}
	}
	...
	render() {
		...
		return (
			<Row style={{ display: this.props.displayDeploy ? 'block' : 'none' }}>
				<Col span={24}>
...
const DepConfig = Form.create()(DeployConfig);
export default props => (
	<CreateAppContext.Consumer>
		{appContext =>
			<AddContainerContext.Consumer>
				{containerContext => <DepConfig  {...appContext} {...props} {...containerContext} />}
			</AddContainerContext.Consumer>
		}
	</CreateAppContext.Consumer>
)